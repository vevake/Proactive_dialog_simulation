...
class DialogState(State):
    ...
    def yield_floor(self, actions):
        if type(actions) is list:
            ...
            last_action = actions
        return last_action.act in [SystemAct.REQUEST, SystemAct.EXPLICIT_CONFIRM, SystemAct.QUERY, SystemAct.RESTART]
    ...
    def state_update(self, usr_actions, conf):
        ...
        for action in usr_actions:
            ...
            if action.act == UserAct.CONFIRM:
                ...
                self.state.atleast_1_slot = True
            ...
            elif action.act == UserAct.RESTART:
#                 self.state.reset_sys_goals()
#                 self.state.reset_slots()
                self.state = DialogState(self.domain)
...
#         import pdb
#         pdb.set_trace()        
        #if restarting the search
#             return [Action(SystemAct.REQUEST, (BaseUsrSlot.NEED, None))], ([], [])

        last_usr = self.state.last_actions(DialogState.USR)
        # dialog opener
        if len(self.state.history) == 0:
            if not self.state.to_restart():
                return [Action(SystemAct.GREET), Action(SystemAct.REQUEST, (BaseUsrSlot.NEED, None))], ([], [])
            else:
                last_usr = [Action(UserAct.REQUEST)]
...
#             print(usr_act)
        ...
        constraints = [(key, slot.get_maxconf_value()) for key, slot in self.state.usr_beliefs.items()]
        cols = {'food':0, 'area':1, 'pricerange':2}
        stat_query = [None, None, None]
        for c in constraints:
            a, b = c
            if a == '#area':
                stat_query[cols['area']] = b
            elif a == "#pricerange":
                stat_query[cols['pricerange']] = b
            else:
                stat_query[cols['food']] = b
                
#         if random.random() <= 0.1:
#             advice = self.domain.db.get_advice(stat_query)
#             if self.state.atleast_1_slot and (len(advice)==0):
#                 return Action(SystemAct.RESTART), ([], [])            
#         else:
        advice = {}
#         print('\nquery:{}, advice:{}'.format(stat_query, self.domain.db.get_advice(stat_query)))
        
#         print(self.state.atleast_1_slot, len(advice))            
        ...
        if self.state.has_pending_return():
            ...
            return actions, (stat_query, advice)
...
#             print(self.state.usr_beliefs.values(), self.state.sys_goals.values())
            for goal in self.state.sys_goals.values():
                if BeliefGoal.THRESHOLD > goal.get_conf() > 0:
            ...
            if len(exp_confirms) > 0:
                ...
                return implicit_confirms, (stat_query, advice)
...
#             print("\nSys_action: ", turn_actions)
            if self.state.is_terminal():
                self.state.update_history(self.state.SYS, turn_actions)
                return 0.0, True, turn_actions, state, stat_query

            if self.state.yield_floor(turn_actions):
                self.state.update_history(self.state.SYS, turn_actions)
                return 0.0, False, turn_actions, state, stat_query
