...
class User(Agent):
    ...
    def __init__(self, domain, complexity, advice_prob=1.0):
        ...
        self.goal_ptr = 0
        self.usr_constrains, self.sys_goals, self.usr_cons_readable = self._sample_goal()
#         print(self.usr_cons_readable)
        self.state = self.DialogState(self.sys_goals)
        self.advice_prob = advice_prob
...
#         exit()
        ...
        usr_cons_readable = {s.name: s.vocabulary[temp_constrains[i]] for i, s in enumerate(self.domain.usr_slots)}
        ...
        return usr_constrains, sys_goals, usr_cons_readable
    ...
    def _increment_goal(self):
        if self.goal_ptr >= self.goal_cnt-1:
            ...
            self.goal_ptr += 1
            _, self.sys_goals, _ = self._sample_goal()
    ...
    def policy(self):
        ...
        if top_action.act == SystemAct.GREET:
        ...
        elif top_action.act == SystemAct.RESTART:
            return Action(UserAct.RESTART)
            ...
            results = {}
            if len(valid_entries)>0:#chosen_entry.shape[0] > 0:
...
#                 print(chosen_entry)
                self.state.spk_state = self.DialogState.LISTEN
                self.usr_constrains, self.sys_goals, _ = self._sample_goal()
                return Action(UserAct.RESTART, [query, results])
#                 raise ValueError("No valid entries")
    ...
    def step(self, inputs, suggestions):
        """
        ...
        """
#         print('usr_cons: ', self.usr_constrains, suggestions)
        for s, v in suggestions.items():
            if random.random() <= self.advice_prob:
                self.usr_constrains[s] = v
#                 print('accepted:', s)
#         print('usr_cons_aftr: ', self.usr_constrains) 
